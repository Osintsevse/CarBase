...
@Service
public class ValvePerCylinderTagFilter extends AbstractTagFilter {
    public ValvePerCylinderTagFilter() {
        super(SimilarityTag.VALVE_PER_CYLINDER);
    }

    @Override
    public SimilarityPredicateAndGroupElement getPredicate(Modification modification, Root<Modification> root) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        Integer valueForComparison = modification.getEngineModification().getEngine().getValvePerCylinder();
        if (valueForComparison != null) {
            Path<Integer> objectPath = root.get(Modification.Fields.engineModification)
                    .get(EngineModification.Fields.engine).get(Engine.Fields.valvePerCylinder);
