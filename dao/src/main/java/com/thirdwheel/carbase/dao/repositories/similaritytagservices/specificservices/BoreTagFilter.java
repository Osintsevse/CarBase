...
@Service
public class BoreTagFilter extends AbstractTagFilter {
    public static final int DEVIATION_PERCENT = 10;

    public BoreTagFilter() {
        super(SimilarityTag.BORE);
    }

    @Override
    public SimilarityPredicateAndGroupElement getPredicate(Modification modification, Root<Modification> root) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        Double valueForComparison = modification.getEngineModification().getEngine().getBore();
        if (valueForComparison != null) {
            ...
            Path<Double> objectPath = root.get(Modification.Fields.engineModification)
                    .get(EngineModification.Fields.engine).get(Engine.Fields.bore);
